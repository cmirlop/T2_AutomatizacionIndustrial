...
\newcommand{\dibujaTransicion}[2]{%
	\coordinate (C#1) at (#1.south |- #2.west);
	\draw[line width = \transitionsLineWidth, -] ([xshift=-\dT]C#1) -- ([xshift=+\dT]C#1);
	}

\newcommand{\dibujaTransicionVertical}[2]{%
	\coordinate (CC#1) at ([yshift=-\retornoDist]#1.south -| #2.north);
	\draw[line width = \transitionsLineWidth, -] ([yshift=-\dT]CC#1) -- ([yshift=+\dT]CC#1);
	}

\newcommand{\dibujaTransicionVerticalXIzq}[2]{% Cas especial: per enllaçar sobre una tornada
	\coordinate (CC#1) at ([yshift=-\retornoDist]#1 -| #2);
	\draw[line width = \transitionsLineWidth, -] ([yshift=-\dT]CC#1) -- ([yshift=+\dT]CC#1);
	}

\newcommand{\transicion}[3]{%
	\draw[line width = \transitionsLineWidth, -] (X#1) --
		node[right, pos=\transitionPos, xshift=1em] (T#2) {#3} 
		(X#2);
	\dibujaTransicion{X#1}{T#2}
	}
...
\newcommand{\etapaFuente}[1]{% Etapa normal sense transció prèvia
   	\node[block] (X#1) {};
	\node[below=0.25ex,anchor=south] at (X#1) {#1};
	}
...
\newcommand{\accion}[3]{\primeraAccion{#1}{#2}{#3}}
...
\newcommand{\forzado}[3]{%
   	\node[action, right of = X#1, node distance = 3em, right] (#2) {#3};
	\draw[line width = \blockLineWidth] ($(#2.north west)+(0.3em,-0.3em)$) rectangle 
		($(#2.south east)-(0.3em,-0.3em)$);
   	\draw[line width = \transitionsLineWidth, -] (X#1) -- (#2);
}
...
\newcommand{\bifurcacion}[6][\bifDistX]{%
	...
	\draw[line width = \transitionsLineWidth, -] (X#2) |- (cantoX#3) -- node[right, xshift=1em] (T#3) {#4} (X#3);
	\dibujaTransicion{cantoX#3}{T#3};
	...
	\draw[line width = \transitionsLineWidth, -] (X#2) |- (cantoX#5) -- node[right, xshift=1em] (T#5) {#6} (X#5);
	\dibujaTransicion{cantoX#5}{T#5};
	}
...
\newcommand{\sincronizacion}[5][\bifDist]{%
	...
	\draw[line width = \transitionsLineWidth, -] (X#2) -- node[right, xshift=1em] (T#2) {#3} (SyncX#2);
	\dibujaTransicion{X#2}{T#2};
...
\newcommand{\concurrencia}[4]{%
	...
	\node[below=0.25ex,anchor=south] at (X#4) {#4};
	\draw[line width = \transitionsLineWidth, -] (SyncX#4) -- node[right, xshift=1em] (T#4) {#3} (X#4);
	\dibujaTransicion{SyncX#4}{T#4};
...
\newcommand{\concuSinco}[5]{%
	...
	\draw[line width = \transitionsLineWidth, -] (SyncA) -- node[right, xshift=1em] (T#4#5) {#3} (SyncB);
	\draw[line width = \transitionsLineWidth, -, double, double distance = 0.2em] ($(X#1)-(3em,\sincDistYbelow)$) -- ($(X#2)-(-3em,\sincDistYbelow)$);

	\dibujaTransicion{SyncA}{T#4#5};
...
\newcommand{\retornoInicio}[4][4em]{% Només per l'esquerra...
	...
	\draw[line width = \transitionsLineWidth, ->, arrows={-Triangle[angle=45:10pt]}] 
		(X#2.south) |- node[right, pos=\transitionPosRetorno, xshift=1em] (T#3) {#4}
...
\newcommand{\retornoInicioNF}[4][4em]{% Només per l'esquerra NO FLETXA
	...
	\node[signal] at ($(X#2 |- dif)-(#1,0em)$) (flech){};
	\draw[line width = \transitionsLineWidth, -] 
		(X#2.south) |- node[right, pos=0.25, xshift=1em] (T#3) {#4}
		($(X#2.south -| flech)-(0em,\retornoDist)$) --
		(flech);
	\draw[line width = \transitionsLineWidth, -] 
		([yshift=-5pt]flech) --
...
\newcommand{\retornoHorizontal}[4][12em]{% Només per la dreta...
	...
	\draw[line width = \transitionsLineWidth, -] 
		([yshift=-6pt]flech) |-
		($(X#3.north)-(0em,-\enlaceRetornoDist)$);
	\dibujaTransicionVertical{X#2}{T#3};
	}

%\retornoHorizontal[12ex]{17}{11}{CondicioDeTransicio}
\newcommand{\saltoHorizontal}[4][12em]{% Només per la dreta...
	\node[signal] at ($(X#2) !0.5! (X#3)$) (dif) {};
	\node[signal] at ($(X#2 |- dif)-(-#1,0em)$) (flech) {};
	\draw[line width = \transitionsLineWidth] 
		(X#2.south) |-
...
\newcommand{\retornoHorizontalX}[4][12em]{% Només per la dreta...
	...
	\dibujaTransicionVertical{#2}{T#3};
	}
...
\newcommand{\retornoHorizontalXIzqZ}[3][12em]{% Només per l'esquerra. Va fins a una de pujada ja existent.
	...
	\dibujaTransicionVerticalXIzq{#2}{T#2};
	}
